...
public class AsyncAppender extends AppenderSkeleton implements
	...
	// static Category cat =
	// Category.getInstance(AsyncAppender.class.getName());
	...
	private ConditionAnnotation c1 = new ConditionAnnotation(bf) {
		...
		public boolean isConditionTrue() {
			...
			return ((BoundedFIFO) o).isFull();
		}
	};
	private ConditionAnnotation c2 = new ConditionAnnotation(bf) {
	 ...
	 * Increasing the size of the buffer is always safe. However, if an existing
	 * buffer holds unwritten elements, then
	 * <em>decreasing the buffer size will result in event
	 * loss.</em> Nevertheless, while script configuring the AsyncAppender, it
	 * is safe to set a buffer size smaller than the
	 * {@link #DEFAULT_BUFFER_SIZE default buffer size} because configurators
	 * guarantee that an appender cannot be used before being completely
	 * configured.
	 * */
...
class Dispatcher extends Thread {
	...
	private ConditionAnnotation c3 = new ConditionAnnotation(bf) {
		...
		public boolean isConditionTrue() {
			...
			return ((BoundedFIFO) o).length()==0;
		}
	};
	private ConditionAnnotation c4 = new ConditionAnnotation(bf) {
		...
		public boolean isConditionTrue() {
			...
			return ((BoundedFIFO) o).wasFull();
		}
	};
	private ConditionAnnotation c5 = new ConditionAnnotation(bf) {
		...
		public boolean isConditionTrue() {
			...
			return ((BoundedFIFO) o).length()==0&&!interrupted;
		}
	};

 ...
 * @version $Revision: 778428 $ $Date: 2009-05-25 10:34:21 -0400 (Mon, 25 May
 *          2009) $
 * @since Pool 1.0
 */
public class GenericObjectPool extends BaseObjectPool implements ObjectPool {
	...
	/**
	 * Allocate available instances to latches in the allocation queue. Then set
	 * _mayCreate to true for as many additional latches remaining in queue as
	 * _maxActive allows.
	 */
	private synchronized void allocate() {
		...
		for (;;) {
			if ((!_allocationQueue.isEmpty())
					&& (_maxActive < 0 || (_numActive + _numInternalProcessing) < _maxActive)) {
				Latch latch = (Latch) _allocationQueue.removeFirst();
	...
	/**
	 * <p>
	 * Invalidates the given object instance. Decrements the active count and
	 ...
	 * 
	 * <p>
	 * Validates the object if testOnReturn == true and passivates it before
	 * returning it to the pool. if validation or passivation fails, or maxIdle
	 * is set and there is no room in the pool, the instance is destroyed.
	 * </p>
	 * 
	 * <p>
	 ...
	 * 
	 * @param obj
	 *            instance to add to the pool
	 * @param decrementNumActive
	 *            whether or not to decrement the active count
	 * @throws Exception
	 */
	 ...
	 * {@link #invalidateObject(Object)} will continue to work. This method does
	 * not {@link #clear()} the pool. The method is idempotent - that is, it is
	 * OK to call it on a closed pool.
	 * 
	 * @throws Exception
	 */
	...
	public void evict() throws Exception {
		...
		for (int i = 0, m = getNumTests(); i < m; i++) {
			...
			synchronized (this) {
				...
				pair = _lifo ? (ObjectTimestampPair) _evictionCursor.previous()
			...
			if ((getMinEvictableIdleTimeMillis() > 0)
				...
				removeObject = true;
					...
					&& ((getNumIdle() + 1) > getMinIdle())) { // +1 accounts for
																// object we are
																// processing
				removeObject = true;
			}
	 ...
	 * cycle. This will ensure that the minimum number of idle instances is
	 * maintained without going past the maxActive value.
	 *
	 * @param incrementInternal
	 *            - Should the count of objects currently under some form of
	 *            internal processing be incremented?
	...
	private synchronized int calculateDeficit(boolean incrementInternal) {
		...
		if (_maxActive > 0) {
			int growLimit = Math.max(0, getMaxActive() - getNumActive()
					- getNumIdle() - _numInternalProcessing);
	...
	private int getNumTests() {
		if (_numTestsPerEvictionRun >= 0) {
			...
			return (int) (Math.ceil(_pool.size()
